...
export function ConversationContent({ content }: { content: AuthoredContent }) {
  if (content.apiCallPlan) {
    return (
      <FeedContent contentId={content.id} apiCallPlan={content.apiCallPlan} />
    )
  } else {
    return (
      <div
      ...
      >
        ...
        {content.role !== "user" && (
          <div>
            <span className={clsx("flex-1")} key={content.id}>
              <Markdown remarkPlugins={[remarkGfm]}>{content.message}</Markdown>
            </span>
          </div>
        )}
...
export function ConversationsPage() {
  ...
  conversation.content.reduce(
    (acc: AuthoredContent[], content: AuthoredContent, index: number) => {
      if (acc.length === 0) {
        return [content]
      }
      if (acc.length === 1) {
        ...
        contentCards.push(
          ...
          >
            <MapContentToCardSection
              content={acc[0]}
              ref={sections[index - 1].ref as RefObject<HTMLDivElement>}
            />
            <MapContentToCardSection
              content={content}
              ref={sections[index]?.ref as RefObject<HTMLDivElement>}
            />
          </motion.div>,
        )
        return []
